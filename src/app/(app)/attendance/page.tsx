...
export default function AttendancePage() {
  ...
  return (
    <div>
      {me?.field_tracking_enabled && <div className={`mb-4 flex items-start gap-3 rounded-xl border p-3 text-xs leading-5 ${checkedIn && !checkedOut ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0"/><div><b>{checkedIn && !checkedOut ? "On Duty · Location tracking eligible" : "Employee Off Duty"}</b><br/>{checkedIn && !checkedOut ? "For tracking-enabled roles, GPS may be recorded according to company policy until Attendance OUT." : "After Attendance OUT, SM HRMS does not store new employee GPS coordinates."} Attendance punch time is generated by the server; changing the device clock cannot backdate IN/OUT.</div></div>}
      <FadeIn>
        <PageHeader title="Attendance" subtitle="Mark your attendance and review records."
          action={
            <Link href="/attendance/register"
              className="flex shrink-0 items-center gap-2 rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700/50">
              <CalendarRange className="h-4 w-4" /> Monthly register
            </Link>
          } />
      </FadeIn>
